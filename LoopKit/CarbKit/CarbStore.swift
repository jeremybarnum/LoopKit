...
extension CarbStore {
    ...
    public func addCarbEntry(_ entry: NewCarbEntry, syncIdentifier: String, completion: @escaping (_ result: CarbStoreResult<StoredCarbEntry>) -> Void) {
        queue.async {
            var result: CarbStoreResult<StoredCarbEntry>?
            var inserted = false
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                do {
                    ...
                    result = .failure(.coreDataError(coreDataError))
                }
            }
